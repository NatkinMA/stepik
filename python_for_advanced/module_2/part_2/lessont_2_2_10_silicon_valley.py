...
В первой строке подаётся число n – количество холодильников. В последующих nn строках вводятся
строки, содержащие латинские строчные буквы и цифры, в каждой строке от 55 до 100100 символов.
...
def anton(chars, text):
    if len(chars) == 1:
        return chars in text
    elif chars[0] in text:
        return anton(chars[1:], text[text.find(chars[0]):])
    else:
        return False
